...
class BlockBreaker(Computer):
    ...
    def GetInput(self) -> int:
        ballPos = [key for key, value in self.screen.items() if value == 4][0]
        ballX = ballPos[0]
        paddlePos = [key for key, value in self.screen.items() if value == 3][0]
        paddleX = paddlePos[0]
        return -1 if ballX < paddleX else 1 if ballX > paddleX else 0
    ...
    def Output(self, value: int) -> None:
        if self.cycle == 0:
            ...
            if self.currX == -1 and self.currY == 0:
    ...
    def Final(self) -> None:
        print(f"Last score: {self.currScore}")
