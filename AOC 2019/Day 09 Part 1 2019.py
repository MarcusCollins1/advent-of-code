...
class Computer:
    ...
    def Run(self, prog: list[int]|None = None, resetIndex: bool = True):
        ...
        if resetIndex:
            index = 0
        while self.data[index] != 99:
            opcode, mode1, mode2, mode3 = self.ParseInstruction(self.data[index])
            # Add
            if opcode == 1:
                op1, op2, op3 = self.GetDatum(index+1), self.GetDatum(index+2), self.GetDatum(index+3)
            ...
            # Jump-if
                ...
                op1, op2 = self.GetDatum(index+1), self.GetDatum(index+2)
                if self.ReadValue(op1, mode1) != 0:
                    index = self.ReadValue(op2, mode2)
                else:
                    index += 3
            # Jump-if-not
            elif opcode == 6:
