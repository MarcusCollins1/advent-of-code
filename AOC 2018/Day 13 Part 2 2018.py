...
# FILE_NAME = "Day 13 2018 test1.txt"
...
while len(carts) > 1:
    ...
    carts = sorted(carts, key=lambda x: (x.numMoves, x.pos))
    currCart = carts[0]
    currCart.move(tracks)
    maxNumMoves = max(maxNumMoves, currCart.numMoves)

    otherCartPos = [cart.pos for cart in carts[1:]]
    if currCart.pos in otherCartPos:
        # print(f"Crash at {currCart.x}, {currCart.y}", end="")
        carts = [cart for cart in carts if cart.pos != currCart.pos]

    # print()

finalCart = carts[0]
while finalCart.numMoves < maxNumMoves:
    finalCart.move(tracks)

print(f"{finalCart.x},{finalCart.y}")
