...
while flag:
    ...
    # check if all cards have won
    num_won = 0
    num_cards = len(data)
    for card in data:
        if has_won(card):
            num_won += 1
    if (num_won + 1) == num_cards:
        flag = False
            

for card in data:
    if has_won(card) == False:
        last_card = deepcopy(card)
        break

flag = True
while flag:
    curr_num = nums.pop(0)
    for row_index in range(len(last_card)):
        curr_row = deepcopy(last_card[row_index])
        for num_index in range(len(curr_row)):
            if curr_row[num_index] == curr_num:
                curr_row[num_index] = "x"
        last_card[row_index] = deepcopy(curr_row)
    if has_won(last_card):
        flag = False

total = 0
for row in last_card:
    for num in row:
        if num != "x":
            total += int(num)
